...
estac_ids <- c( "21023", "21025", "21026", "21027", "21031", "21033", "21038", "21039", "21040", "21052", "21053", "21056", "21060", "21067", "21072", 
...
for (id in estac_ids) {
  path_era5 <- read.csv2(paste0(dir_era5, "ERA5_extracted_50-25_", id, "_t2m.csv"))
  path_era5[c("date")] <- as.Date(path_era5$date)
  path_era5[c("value")] <- as.numeric(path_era5$value)
  
  path_smn <- read_csv(paste0(dir_smn, "SMN_month_", id, ".csv"))
  path_smn$date <- as.Date(paste(path_smn$Anio, path_smn$Mes, "01", sep = "-"))
  path_smn$Tmed <- as.numeric((path_smn$TMAX_prom+path_smn$TMIN_prom)/2)
  ...
  ggplot(data = era5_smn, aes(x=Fecha)) +
    ...
    labs( x = "Fecha",
          ...
          title = paste("Series de tiempo de temperatura media mensual para la estación", id),
          subtitle = "Fuente de datos: ERA5 y SMN")+
    theme_gray()+
    theme(legend.position = "top",
              plot.title = element_text(face = "bold", size = 14))
              
  ggsave(paste("./03_OUTPUTS/Imagenes/t2m/Serie_ERA5vsSMN_",id,"_t2m.png"),
    plot = last_plot(), 
    width = 11, height = 8.5,
    units = "in", dpi = 300)
}
