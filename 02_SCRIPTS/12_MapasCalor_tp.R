...
for (id in estac_ids) {
  ...
  path_era5 <- paste0(dir_era5, "ERA5_extracted_50-25_", id, "_tp.csv")
...
p_final <- ggplot(df_mapa_calor, aes(x = date, y = estacion, fill = status)) +
  ...
  scale_fill_manual(values = c(
    "Solo ERA5"       = "#D8BA98",
    "Solo SMN"        = "#96C0CE",  # Azul
    "Coinciden ambas" = "#0F414A",  # Verde
    "Sin Datos"       = "#EFE8DF"   # Gris claro para vacíos
  )) +
  # Eje X configurado cada 10 años para dar espacio a series de tiempo largas (desde los 40s)
  scale_x_date(date_breaks = "5 years", date_labels = "%Y") +
  labs(
    title = "Heatmap disponibilidad de registros de precipitacion: ERA5 vs SMN",
    subtitle = paste("Series temporales mensuales (mm/dia) para", length(unique(df_mapa_calor$estacion)), "estaciones"),
    x = "Fechas",
    y = "ID Estación",
    fill = "Disponibilidad"
  ) +
...
ggsave(
    "./03_OUTPUTS/Heatmap_ERA5vsSMN_tp.png", 
    plot = last_plot(), 
    width = 8.5,           # Ancho
    height = 11,           # Alto
